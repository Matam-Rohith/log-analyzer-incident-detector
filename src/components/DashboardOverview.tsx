  ...
  return (
    <div className='space-y-6'>
      <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
        ...
        <StatCard title='Incidents'   value={analytics.incidentCount}   icon={Shield}       color='#ff0040' index={1} />
        ...
        <StatCard title='Sources'     value={analytics.uniqueSources}   icon={Server}        color='#00ff88' index={7} />
      </div>
      ...
      {analytics.timelineData.length > 0 && (
        ...
        >
          <h3 className='text-sm font-semibold text-slate-400 uppercase tracking-widest mb-4'>Log Activity Timeline</h3>
      ...
      {analytics.levelDistribution.length > 0 && (
        <div className='grid md:grid-cols-2 gap-4'>
          <motion.div
            ...
            className='rounded-lg border border-slate-700 bg-slate-900 p-5'
          >
            <h3 className='text-sm font-semibold text-slate-400 uppercase tracking-widest mb-4'>Level Distribution</h3>
            <ResponsiveContainer width='100%' height={200}>
              <PieChart>
                <Pie
                ...
                >
                  {analytics.levelDistribution.map((entry, idx) => (
          ...
          <motion.div
            ...
            className='rounded-lg border border-slate-700 bg-slate-900 p-5'
          >
            <h3 className='text-sm font-semibold text-slate-400 uppercase tracking-widest mb-4'>Top Error Sources</h3>
              ...
              {analytics.sourceDistribution.slice(0, 6).map((s, i) => (
                <div key={s.source} className='flex items-center gap-3'>
                  ...
                  <div className='flex items-center gap-2'>
                    <div className='h-1.5 rounded-full bg-green-400/20 overflow-hidden' style={{ width: 80 }}>
                      <div
                        className='h-full bg-green-400 rounded-full'
                        style={{ width: `${Math.min(100, (s.count / analytics.totalLogs) * 100 * 5)}%` }}
                      />
                    </div>
                    <span className='text-xs text-green-400 font-mono w-8 text-right'>{s.count}</span>
                  </div>
                </div>
              ))}
            </div>
